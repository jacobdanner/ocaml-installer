...
Section "Cygwin" SecCygwin
  ${If} ${FileExists} "$INSTDIR\cygwin-setup.exe"
    MessageBox MB_YESNO "There already is a file called cygwin-setup.exe on your $INSTDIR. Overwrite?" IDNO end
  ${EndIf}

  NSISdl::download ${CYGWIN_URL} "$INSTDIR\cygwin-setup.exe"
